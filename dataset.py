...
from .arguments import (
    EncoderOnlyEmbedderM3SEALModelArguments,
    EncoderOnlyEmbedderM3SEALTrainingArguments,
    EncoderOnlyEmbedderM3SEALDataArguments
)

logger = logging.getLogger(__name__)


class EncoderOnlyEmbedderM3SEALTrainDataset(Dataset):
    ...
    def __init__(
        self,
        args: EncoderOnlyEmbedderM3SEALDataArguments,
...
@dataclass
class EncoderOnlyEmbedderM3SEALCollator(DataCollatorWithPadding):
    """
...
class EncoderOnlyEmbedderM3SEALSameDatasetTrainDataset(EncoderOnlyEmbedderM3SEALTrainDataset):
    ...
    def __getitem__(self, _):
        ...
        self.step += 1
        queries, passages, passages_masked, passages_untagged, teacher_scores = self._create_batch_data(batch_raw_data=batch_data)
        return queries, passages, passages_masked, passages_untagged, teacher_scores, no_in_batch_neg_flag
    ...
    def _mask_element(self, text, mask_ratio=0.1, mask_token=""):
        """
        随机mask HTML标签包含的内容
        
        参数:
        text: 输入的HTML文本
        mask_ratio: 需要mask的标签内容比例(0-1之间)
        mask_token: 用于替换的mask标记
    ...
    def _create_batch_data(self, batch_raw_data):
        ...
        """
        queries, passages, passages_masked, passages_untagged, teacher_scores = [], [], [], [], []
        ...
        for i in range(len(batch_raw_data['query'])):
            ...
            tmp_passages_masked.append(self._mask_element(pos))
            tmp_passages_untagged.append(self._remove_tag(pos))
            ...
            for neg_idx in neg_idxs:
                ...
                tmp_passages_masked.append(self._mask_element(batch_raw_data['neg'][i][neg_idx]))
                tmp_passages_untagged.append(self._remove_tag(batch_raw_data['neg'][i][neg_idx]))
            ...
            if data_type is not None and data_type in ['symmetric_sts', 'symmetric_clustering']:
                ...
                if self.args.passage_instruction_for_retrieval is not None:
                    ...
                    tmp_passages_masked = [
                        self.args.passage_instruction_format.format(
                            self.args.passage_instruction_for_retrieval, p
                        ) for p in tmp_passages_masked
                    ]
                    tmp_passages_untagged = [
                        self.args.passage_instruction_format.format(
                            self.args.passage_instruction_for_retrieval, p
                        ) for p in tmp_passages_untagged
                    ]

            passages.extend(tmp_passages)
            passages_masked.extend(tmp_passages_masked)
            passages_untagged.extend(tmp_passages_untagged)

            if teacher_scores is not None:
                if len(teacher_scores) > 0 and len(passages) > 0:
                    assert len(teacher_scores) == len(passages)

        return queries, passages, passages_masked, passages_untagged, teacher_scores


@dataclass
class EncoderOnlyEmbedderM3SEALSameDatasetCollator(DataCollatorWithPadding):
    """
    ...
    def __call__(self, features):
        ...
        passages_masked = features[0][2]
        passages_untagged = features[0][3]
        teacher_scores = features[0][4]
        ...
        passages_masked_inputs = self.tokenizer(
            passages_masked,
            truncation=True,
            max_length=self.passage_max_len,
            return_tensors=None
        )
        passages_untagged_inputs = self.tokenizer(
            passages_untagged,
            truncation=True,
            max_length=self.passage_max_len,
            return_tensors=None
        ...
        if self.sub_batch_size is None or self.sub_batch_size <= 0:
            ...
            d_masked_collated = self.tokenizer.pad(
                passages_masked_inputs,
                padding=self.padding,
                max_length=self.passage_max_len,
                pad_to_multiple_of=self.pad_to_multiple_of,
                return_tensors=self.return_tensors,
            ...
            d_untagged_collated = self.tokenizer.pad(
                passages_untagged_inputs,
                padding=self.padding,
                max_length=self.passage_max_len,
                pad_to_multiple_of=self.pad_to_multiple_of,
                return_tensors=self.return_tensors
            ...
            d_masked_collated = []
            for i in range(0, len(passages_masked_inputs['attention_mask']), batch_size):
                start = i
                end = min(len(passages_masked_inputs['attention_mask']), i + batch_size)
                sub_features = {}

                for k, b in passages_masked_inputs.items():
                    sub_features[k] = v[start: end]
                d_masked_collated.append(self.tokenizer.pad(
                    sub_features,
                    padding=self.padding,
                    max_length=self.passage_max_len,
                    pad_to_multiple_of=self.pad_to_multiple_of,
            ...
            d_untagged_collated = []
            for i in range(0, len(passages_untagged_inputs['attention_mask']), batch_size):
                start = i
                end = min(len(passages_untagged_inputs['attention_mask']), i + batch_size)
                sub_features = {}

                for k, b in passages_untagged_inputs.items():
                    sub_features[k] = v[start: end]
                d_untagged_collated.append(self.tokenizer.pad(
                    sub_features,
                    padding=self.padding,
                    max_length=self.passage_max_len,
                    pad_to_multiple_of=self.pad_to_multiple_of,
        ...
        return {
            "queries": q_collated,
            "passages": d_collated,
            "passages_masked": d_masked_collated,
            "passages_untagged": d_untagged_collated,
            "teacher_scores": teacher_scores,
