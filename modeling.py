...
class EncoderOnlyEmbedderM3SEALModel(AbsEmbedderModel):
    ...
    def forward(
        ...
        passages_masked: Union[Dict[str, Tensor], List[Dict[str, Tensor]]] = None,
        passages_untagged: Union[Dict[str, Tensor], List[Dict[str, Tensor]]] = None,
        ...
        p_masked_dense_vecs, _, _ = self.encode(passages_masked)
        p_untagged_dense_vecs, _, _ = self.encode(passages_untagged)
        ...
        if self.training:
            ...
            # q_p_masked_dense_scores, q_p_masked_loss = compute_loss_func(
            #     q_dense_vecs, p_masked_dense_vecs, teacher_targets=teacher_targets,
            #     compute_score_func=self.compute_dense_score
            # )
            # p_p_masked_dense_scores, p_p_masked_loss = compute_loss_func(
            #     p_dense_vecs, p_masked_dense_vecs, teacher_targets=teacher_targets,
            #     compute_score_func=self.compute_dense_score
            # )
            q_p_untagged_dense_scores, q_p_untagged_loss = compute_loss_func(
                q_dense_vecs, p_untagged_dense_vecs, teacher_targets=teacher_targets,
                compute_score_func=self.compute_dense_score
            )
            # p_p_untagged_dense_scores, p_p_untagged_loss = compute_loss_func(
            #     p_dense_vecs, p_untagged_dense_vecs, teacher_targets=teacher_targets,
            #     compute_score_func=self.compute_dense_score
            # )
        ...
        return EmbedderOutput(
            # loss = dense_loss
            # loss=dense_loss+q_p_masked_loss+p_p_masked_loss,
            # loss = dense_loss + q_p_masked_loss
            # loss = dense_loss + p_p_masked_loss
            loss = dense_loss + q_p_untagged_loss
            # loss = dense_loss + p_p_untagged_loss
            # loss = dense_loss + q_p_untagged_loss + p_p_untagged_loss
            # loss = dense_loss + q_p_masked_loss + q_p_untagged_loss
            # loss = dense_loss + q_p_masked_loss + p_p_untagged_loss
        )
...
class EncoderOnlyEmbedderM3SEALModelForInference(EncoderOnlyEmbedderM3SEALModel):
    """
